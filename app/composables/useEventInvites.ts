...
export function useEventInvites(eventId: MaybeRefOrGetter<string | null>) {
  ...
  async function withProfileNames(
    ...
    const nameByEmail = new Map(
      (profiles ?? [])
        .filter((p) => p.email)
        .map((p) => [p.email, p.display_name]),
    );
    return candidates.map((c) => ({
      email: c.email,
      display_name: nameByEmail.get(c.email) ?? c.display_name,
    }));
  }
  ...
  async function seedFromLastEvent(): Promise<number> {
    ...
    if (otherIds.length) {
      ...
      const sourceId = otherIds.find((eid) =>
        (pool ?? []).some((p) => p.event_id === eid),
      );
      if (sourceId) {
        candidates = (pool ?? [])
    ...
    const named = await withProfileNames(
      candidates.filter((c) => !existing.has(c.email)),
    );
    const toAdd = named.map((c) => ({
      event_id: id,
      email: c.email,
  ...
  async function sendInvites(): Promise<{
    sent: number;
    failed: number;
    error: string | null;
  }> {
    ...
    const result = await $fetch<{
      ...
      failed: number;
      error: string | null;
    }>(`/api/events/${id}/invites/send`, { method: "POST" });
    await refresh();
